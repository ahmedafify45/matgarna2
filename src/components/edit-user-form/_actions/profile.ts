...
// ✅ دالة لحذف الصورة القديمة من Cloudinary
...
// ✅ استخراج `public_id` الصحيح من رابط الصورة
const getPublicIdFromUrl = (url: string) => {
  const parts = url.split("/");
  const fileName = parts.pop()?.split(".")[0]; // استخراج الاسم بدون الامتداد
  const folder = parts.includes("profile_images") ? "profile_images/" : "";
